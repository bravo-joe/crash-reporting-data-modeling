...
Step 1a: Create splits, folds, performance metrics, and control object
```{r}
# Splits
...
rf_rec <-
  ...
  step_upsample(injury_severity, over_ratio = 0.53)
rf_rec
```
Step 1c: Model Specifications
```{r}
rf_model <-
  rand_forest(
      mtry = tune(),
...
stopCluster(cluster)
```
Step 1f: Collect performance metrics
```{r}
