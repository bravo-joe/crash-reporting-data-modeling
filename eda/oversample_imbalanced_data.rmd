...
rf_model_wflow
```
Step 3b: Fit workflow and data to model
...
rf_model_fit
```
Step 4b: Use the 'predict()' function to use validation (test) data
```{r}
library(caret)
pred1 <- predict(rf_model_fit, new_data = test_data)
pred1
```
Step 5b: Extract the performance metrics
```{r}
