...
```
### Prototype the Random Forest model - Take 4
This iteration will use parallel processing and difference resampling approach. 
This step should require about 6 steps.
```{r}
# Step 4a: Recipe
rf_recipe_4 <-
...
```
Resampling with parallel processing
```{r}
...
# Step 4e: Resampling
# Performance metrics
perf_metrics_4 <- metric_set(roc_auc, accuracy)
# random forest resample
rf_rs_4 <- rf_model_wflow_4 %>%
  fit_resamples(
    resamples = severity_folds,
    metrics = perf_metrics_4,
    control = control_resamples(save_pred = TRUE)
  )
#stopCluster(cluster)
rf_rs_4
...
### Prototype the kNN model - Take 1
Step 1a: Initiate the recipe
```{r}
knn_recipe <-
  recipe(injury_severity  ~ ., data = train_data) %>%
